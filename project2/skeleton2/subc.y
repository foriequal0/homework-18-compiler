...
%left                   '+' '-'
%left                   '*' '/' '%'
%right                  '!' PLUS_PLUS MINUS_MINUS UMINUS UAST UAMP
%left                   STRUCTOP

/* Token and Types */
%token                  TYPE STRUCT STRUCTOP
%token                  RETURN IF ELSE WHILE FOR BREAK CONTINUE
%token                  ASSIGNOP LOGICAL_OR LOGICAL_AND RELOP EQUOP
...
ext_def_list
    ...
    ;
ext_def
    : opt_specifier ext_decl_list ';' { REDUCE("ext_def->opt_specifier ext_decl_list ';'"); }
    | opt_specifier funct_decl compound_stmt { REDUCE("ext_def->opt_specifier funct_decl compound_stmt"); }
    ;
ext_decl_list
    : ext_decl { REDUCE("ext_decl_list->ext_decl"); }
    | ext_decl_list ',' ext_decl { REDUCE("ext_decl_list->ext_decl_list ',' ext_decl"); }
    ;
ext_decl
    : var_decl { REDUCE("ext_decl->var_decl"); }
    | funct_decl { REDUCE("ext_decl->funct_decl"); }
    ;
opt_specifier
    : type_specifier { REDUCE("opt_specifier->type_specifier"); }
...
var_decl
    ...
    ;
funct_decl
    : ID '(' ')' { REDUCE("funct_decl->ID '(' ')'"); }
    | ID '(' var_list ')' { REDUCE("funct_decl->ID '(' var_list ')'"); }
    // When we declare a function with ID(),
    // we want to have a function which has no parameter.
    ;
var_list
    : param_decl { REDUCE("var_list->param_decl"); }
    | var_list ',' param_decl { REDUCE("var_list->var_list ',' param_decl"); }
    ;
param_decl
    : type_specifier var_decl { REDUCE("param_decl->type_specifier var_decl"); }
    ;
def_list
    ...
    ;
def
    : type_specifier decl_list ';' { REDUCE("def->type_specifier decl_list ';'"); }
    ;
decl_list
    : decl_list ',' decl { REDUCE("decl_list->decl_list ',' decl"); }
    | decl { REDUCE("decl_list->decl"); }
    ;
decl
    : funct_decl { REDUCE("decl->funct_decl"); }
    | var_decl { REDUCE("decl->var_decl"); }
    ;
compound_stmt
    : '{' local_defs stmt_list '}' { REDUCE("compound_stmt->'{' local_defs stmt_list '}'"); }
    ;
local_defs
    : def_list { REDUCE("local_defs->def_list"); }
    ;
stmt_list
...
stmt
    ...
    ;
test
    : expr { REDUCE("test->expr"); }
    | /* empty */ { REDUCE("test->epsilon"); }
    ;
opt_expr
    : expr { REDUCE("opt_expr->expr"); }
    | /* empty */ { REDUCE("opt_expr->epsilon"); }
    ;
expr
    : expr ASSIGNOP expr { REDUCE("expr->expr ASSIGNOP expr"); }
    | expr '=' expr { REDUCE("expr->expr '=' expr"); }
    | or_expr { REDUCE("expr->or_expr"); }
    ;
or_expr
    : or_list { REDUCE("or_expr->or_list"); }
    ;
or_list
    : or_list LOGICAL_OR and_expr { REDUCE("or_list->or_list LOGICAL_OR and_expr"); }
    | or_list '|' and_expr { REDUCE("or_list->or_list '|' and_expr"); }
    | and_expr { REDUCE("or_list->and_expr"); }
    ;
and_expr
    : and_list { REDUCE("and_expr->and_list"); }
    ;
and_list
    : and_list LOGICAL_AND binary { REDUCE("and_list->and_list LOGICAL_AND binary"); }
